# Описание работы с моей реализацией bookkeeper

python3.10 ./bookkeeper/Presenter.py - запуск программы

В главном окне программы Вы можете найти пустую таблицу с расходами, которая будет заполняться по мере их добавления.
Ниже следует окно бюджета, который пользователь задает и обнуляет в ручную. При выходе значения в поле "Сумма" за пределы
бюджета кнопка добавить блокируется.

Ниже поля бюджета следует поле ввода. Для добавления покупки необходимо выбрать категорию, ввести сумму и нажать 
"Добавить". В окно ввода суммы можно ввести только численные значения.

В самой нижней части интерфейса находится кнопка "Категории". При нажатии на нее открывается таблица с доступными
категориями и индексами их родиетелй. Их можно свободно редактировать. Колонка с индексами от редактирования заблокирована.
Если пользователь хочет добавить категорию, он может ввести информацию о ней в свободную нижнюю ячейку таблицы.

P.S. я честно не понял, что это за 1 непокрытый тестами элемент программы, но я очень старался.
